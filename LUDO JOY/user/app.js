...
function showDemoLogin() {
    document.getElementById('demo-fields').style.display = 'block';
}

function loginDemo() {
    const name = document.getElementById('demo-name').value.trim() || 'Demo User';
    showLoading();
    const demoUid = 'demo_' + Date.now();
    const demoUser = {
        name: name,
        email: 'demo@ludojoy.app',
        photoURL: '',
    ...
    db.collection('users').doc(demoUid).set(demoUser).then(() => {
        currentUser = { id: demoUid, ...demoUser };
        afterLogin();
    }).catch(() => {
        currentUser = { id: demoUid, ...demoUser };
        afterLogin();
