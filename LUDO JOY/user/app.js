    ...
    db.collection('users').where('email', '==', user.email).get().then(snap => {
        if (snap.empty) {
            ...
            db.collection('users').doc(user.uid).set(newUser).then(() => {
                ...
                hideLoading();
                showToast('Account create failed: ' + err.message, '#ff3b30');
            });
        } else {
        ...
        hideLoading();
        showToast('Database error: ' + err.message, '#ff3b30');
...
function navigateTo(sectionId) {
    ...
    if (sectionId === 'deposit-page-section') { loadDepositOptionsPage(); }
    if (sectionId === 'withdraw-page-section') { checkKYCStatusPage(); }
}

function goToDeposit() {
    document.getElementById('deposit-balance').textContent = currentUser.balance || 0;
...
function loadDepositOptionsPage() {
    const container = document.getElementById('deposit-opts-page');
    const options = [100, 200, 300, 400, 500, 1000, 2000, 5000];
    container.innerHTML = options.map(o => `
        <div class="do-item" onclick="selectDepositPage(${o}, this)">₹${o}</div>
    `).join('');
}
function selectDepositPage(amount, el) {
    document.querySelectorAll('#deposit-opts-page .do-item').forEach(d => d.classList.remove('selected'));
    el.classList.add('selected');
    selectedDepositPage = amount;
    document.getElementById('custom-deposit-page').value = amount;
}
function depositMoneyPage() {
    let amount = parseFloat(document.getElementById('custom-deposit-page').value) || selectedDepositPage;
    if (amount < 100) { showToast('Minimum deposit ₹100', '#ff3b30'); return; }
    simulateDeposit(amount);
}

// ==================== WITHDRAW PAGE ====================
function checkKYCStatusPage() {
    if (!currentUser) return;
    db.collection('kyc_requests').where('userId', '==', currentUser.id).orderBy('timestamp', 'desc').limit(1).get().then(snap => {
        if (!snap.empty) {
            const data = snap.docs[0].data();
            if (data.status === 'approved') {
                document.getElementById('kyf-section-page').style.display = 'none';
                document.getElementById('withdraw-form-page').style.display = 'block';
            } else if (data.status === 'pending') {
                document.getElementById('kyf-section-page').innerHTML = '<div style="text-align:center;padding:20px;color:var(--warning);font-weight:600;">KYC Pending Approval</div>';
            }
        }
    }).catch(() => {});
}
function toggleWithdrawFieldsPage() {
    const method = document.getElementById('withdraw-method-page').value;
    document.getElementById('bank-fields-page').style.display = method === 'bank' ? 'block' : 'none';
    document.getElementById('upi-fields-page').style.display = method === 'upi' ? 'block' : 'none';
}
function submitKYFPage() {
    const name = document.getElementById('kyf-name-page').value.trim();
    const aadhar = document.getElementById('kyf-aadhar-page').value.trim();
    const front = document.getElementById('kyf-front-page').value.trim();
    const back = document.getElementById('kyf-back-page').value.trim();
    if (!name || !aadhar || !front || !back) { showToast('All KYC fields required', '#ff3b30'); return; }
    db.collection('kyc_requests').add({
        userId: currentUser.id,
        userName: currentUser.name,
...
function withdrawMoneyPage() {
    ...
    if (method === 'bank') {
        ...
        const ifsc = document.getElementById('w-ifsc-page').value.trim();
        const acc = document.getElementById('w-account-page').value.trim();
        const confirm = document.getElementById('w-confirm-account-page').value.trim();
        if (!holder || !ifsc || !acc || !confirm) { showToast('All bank fields required', '#ff3b30'); return; }
        if (acc !== confirm) { showToast('Account numbers do not match', '#ff3b30'); return; }
        details = { method: 'bank', accountHolder: holder, ifsc, accountNumber: acc };
    } else {
